...
it(
    'looks up company info on name for dk country using artisan command',
...
->with('dk-companies')
->skip(
    fn () => config('company-info.countries.dk.provider') === 'cvrapi' && config(
        'company-info.providers.cvrapi.user_agent'
    ) === ''
)
->skip(
...
it(
    'looks up company info on name for dk country using artisan command, output as json',
    function (string $name, string $number, array $expected) {
        $this
            ->artisan('company-info:lookup', [
                '--name'    => $name,
...
it(
    'looks up company info on number for dk country using artisan command',
    function (string $name, string $number, array $expected) {
        $this
            ->artisan('company-info:lookup', [
                '--number'  => $number,
